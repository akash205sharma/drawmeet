...
export default function ChatPanel({
  ...
  // Assume current user is last in onlineUsers for demo
  const currentUser = onlineUsers[onlineUsers.length - 1] || "";
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2">
        {messages.map((msg, i) => {
          ...
          return (
            ...
            >
              {!isSelf && (
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${getAvatarColor(msg.user)}`}>{msg.user[0]}</div>
              )}
              <div
