...
DrawMeet uses an event-sourced architecture where every canvas operation is persisted as an immutable action rather than storing the full canvas state. Real-time collaboration is achieved by broadcasting actions through Socket.IO and replaying the event log to reconstruct the latest board for new participants. Since most operations are additive (creating shapes, strokes, text, arrows, and sticky notes), concurrent actions naturally coexist without conflicts. For operations that modify the same object, the current implementation effectively follows a last-writer-wins approach based on server-side action ordering. It does not yet implement advanced conflict-resolution mechanisms such as CRDTs or Operational Transformation
